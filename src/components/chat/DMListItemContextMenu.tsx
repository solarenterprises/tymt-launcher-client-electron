...
const DMListItemContextMenu = ({
  view,
  setView,
  DM,
  contextMenuPosition,
}: IPropsDMListItemContextMenu) => {
  ...
  return (
    <>
      <Modal open={view} onClose={handleOnClose}>
        <Fade in={view}>
          ...
          >
            ...
            <Box
              className={"fs-16 white context_menu_middle"}
              textAlign={"left"}
              sx={{ backdropFilter: "blur(10px)" }}
      ...
      <ExportChatModal
        view={openExportModal}
        setView={setOpenExportModal}
        group={DM}
      />
    </>
  );
