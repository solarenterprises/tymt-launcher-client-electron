...
const DMListItem = ({ DM, index, roomMode, setView }: IPropsDMListItem) => {
  ...
  // const unreadMessageListStore: IUnreadMessageList =
  //   useSelector(getUnreadMessageList);

  // const partnerId =
  //   DM.participants.find((element) => element.userId !== myInfoStore?._id)
  //     ?.userId ?? "";
  // const user = contactListStore.contacts.find(
  //   (element) => element._id === partnerId
  // );
  const user = { avatar: "", nickName: "", sxpAddress: "", notificationStatus: "" };
  ...
  //   try {
  //     return unreadMessageListStore.data.find(
  //       (element) => element.chatroomId === DM._id
  //     ).unreadMessageCount;
  //   } catch (err) {
  ...
  const unreadMessageCountForChatroom = 0;
  ...
  //     await AlertAPI.readAllUnreadAlertsForChatroom({
  ...
  //         instructions: [
  //           SyncEventNames.UPDATE_ALERT_LIST,
  //           SyncEventNames.UPDATE_UNREAD_MESSAGE_LIST,
  //         ],
  //         is_to_self: true,
  //       };
  ...
  const handleDMListItemRightClick = (
    ...
    user && (
      <>
        <Box
          ...
          // onClick={handleDMListItemClick}
          onContextMenu={handleDMListItemRightClick}
        >
          <Grid
            item
            xs={12}
          ...
          >
            <Avatar
              // onlineStatus={activeUserListStore.users.some(
