...
const Bubble = ({
  ...
  const chatHistoryStore: { messages: any[] } = { messages: [] };
  // const currentChatroomStore: IChatroom = useSelector(getCurrentChatroom);
  const currentChatroomStore: { isPrivate: boolean } = { isPrivate: false };
  ...
  const formatDateDifference = (date: any) => {
  ...
  const isFirstMessageOfDay = useCallback(() => {
    ...
    const previousMessageDate = new Date(
      chatHistoryStore.messages[index + 1]?.createdAt
    );
  ...
  return (
    <Box className={"bubblecontainer"} key={`${index}-${message.createdAt}`}>
      {timeline && decryptedMessage !== undefined && (
      ...
      <Stack
        ...
        justifyContent={
          !screenExpanded
            ? message.sender_id === myInfoStore?._id
              ? "flex-end"
              : "flex-start"
            : "flex-start"
        }
        ...
        {roomMode && screenExpanded && isLastMessageOfStack && !isSender && (
          <Avatar
            onlineStatus={activeUserListStore.users.some(
              (user) => user === message.sender_id
            )}
            userid={message.sender_id}
            size={40}
            status={
              currentChatroomMembersStore?.members?.find(
                (member) => member._id === message._id
              )?.notificationStatus
            }
          />
        )}
        {roomMode && screenExpanded && !isLastMessageOfStack && (
