...
const UserListItem = ({
  ...
  const handleUserListItemRightClick = (
  ...
  const handleUserListItemClick = useCallback(async () => {
    ...
    //     if (
    //       blockListStore?.contacts.some(
    //         (element) => element?._id === user._id
    //       ) ||
    //       contactListStore?.contacts.some(
    //         (element) => element?._id === user._id
    //       ) ||
    ...
    //           await dispatch(
    //             fetchCurrentChatroomMembersAsync(newCurrentChatroom?._id)
    //           );
    //           await dispatch(
    //             fetchHistoricalChatroomMembersAsync(newCurrentChatroom?._id)
    //           );
    //         }
    ...
    //             const myUserKey = newCurrentChatroom?.participants.find(
    //               (participant) => participant.userId === myInfoStore?._id
    //             )?.userKey;
    //             dispatch(
    //               addOneSKeyList({
  ...
  return (
    <>
      <Box
        ...
        onContextMenu={handleUserListItemRightClick}
      >
        <Grid
          item
          xs={12}
