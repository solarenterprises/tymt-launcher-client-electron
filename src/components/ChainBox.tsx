...
const ChainBox = ({ supportChain, onClick, onDoubleClick }: IPropsChainBox) => {
  ...
  //   () =>
  //     getTokenBalanceBySymbol(balanceListStore, supportChain?.chain?.symbol) ??
  //     0,
  //   [balanceListStore]
  // );
  // const price = useMemo(
  //   () => getTokenPriceByCmc(priceListStore, supportChain?.chain?.cmc) ?? 0,
  //   [priceListStore]
  // );
      ...
      >
        <Stack gap={2} direction={"row"} justifyContent={"flex-start"}>
          ...
          >
            ...
            <Box className="fs-12-light blue">
        ...
        >
          ...
          <Box className="fs-12-light gray">
            {/* {`${symbol} ${formatBalance(
              Number(price) * Number(balance) * reserve
            )}`} */}
          </Box>
        </Stack>
      </Stack>
